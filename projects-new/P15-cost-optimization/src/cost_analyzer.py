...
class CostAnalyzer:
        ...
        try:
            ...
            for recommendation in response.get("Recommendations", []):
                ...
                recommendations.append(
                    {
                        "instance_type": details["InstanceDetails"][
                            "EC2InstanceDetails"
                        ]["InstanceType"],
                        "region": details["InstanceDetails"]["EC2InstanceDetails"][
                            "Region"
                        ],
                        "estimated_monthly_savings": details[
                            "EstimatedMonthlySavingsAmount"
                        ],
                        "estimated_monthly_on_demand_cost": details[
                            "EstimatedMonthlyOnDemandCost"
                        ],
                        "recommended_instances": details[
    ...
    def generate_report(self, output_file="cost_report.json"):
        ...
        total_idle = sum(
            len(resources) for resources in report["idle_resources"].values()
        )
