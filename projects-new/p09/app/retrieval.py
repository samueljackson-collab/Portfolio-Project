...
class RetrievalService:
    ...
    async def keyword_search(
        self, query: str, vector_db_client, top_k: int = 10
    ) -> List[Dict]:
        """Keyword-based BM25 search (if supported by vector DB)."""
        # Fallback: use metadata filtering or full-text search
        results = await vector_db_client.keyword_search(query=query, limit=top_k)
        return results
    ...
    async def hybrid_search(
        ...
        semantic_results = await self.semantic_search(
            query, vector_db_client, top_k * 2
        )
        ...
        hybrid_results = [
            all_results[doc_id] for doc_id in ranked_ids if doc_id in all_results
        ]
    ...
    def compute_grounding_score(self, answer: str, sources: List[Dict]) -> float:
        """Compute grounding score based on citation presence."""
        # Simple heuristic: check for source references in answer
        has_citation = any(
            source["metadata"]["source"] in answer or f"[{i+1}]" in answer
