# Container Registry Management - Operations Runbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This runbook provides operational procedures, troubleshooting guides, and incident response workflows for Container Registry Management.
...
- **Service Name:** Container Registry Management
...
- **Grafana:** https://grafana.example.com/d/container-registry
- **CloudWatch:** https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#dashboards:name=container-registry
...
**Alert:** `container-registry_high_error_rate`
...
```bash
# Check error logs
aws logs tail /aws/lambda/container-registry --follow --filter '5xx'

# Check service health
kubectl get pods -n container-registry
kubectl logs -n container-registry <pod-name> --tail=100
```
...
**Alert:** `container-registry_high_latency`
...
aws rds describe-db-instances --db-instance-identifier container-registry-prod
...
kubectl top pods -n container-registry
...
```markdown
# Incident Report: container-registry Outage
...
aws s3 ls s3://backups-container-registry/latest/
```
...
cat /etc/logrotate.d/container-registry

# Test log rotation
logrotate -d /etc/logrotate.d/container-registry
```
