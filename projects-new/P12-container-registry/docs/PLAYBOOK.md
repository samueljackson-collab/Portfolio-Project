# Container Registry Management - Implementation Playbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This playbook provides step-by-step instructions for implementing, deploying, and operating Container Registry Management from scratch.
...
cd portfolio/P12-container-registry
```
...
../../scripts/bootstrap_remote_state.sh container-registry us-east-1

# Output will show:
# S3 Bucket: container-registry-tfstate-xxxxx
# DynamoDB Table: container-registry-tfstate-lock
```
...
terraform {
  backend "s3" {
    bucket         = "container-registry-tfstate-xxxxx"  # From bootstrap output
    key            = "container-registry/terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "container-registry-tfstate-lock"
    encrypt        = true
  }
}
```
...
```bash
# Check VPC
aws ec2 describe-vpcs --filters "Name=tag:Name,Values=container-registry-vpc"

# Check RDS
aws rds describe-db-instances --db-instance-identifier container-registry-db

# Check ECS cluster
aws ecs describe-clusters --clusters container-registry-cluster
```
...
# Test locally
docker run -p 8080:8080 --env-file .env container-registry:latest
...
# Tag image
docker tag container-registry:latest <account-id>.dkr.ecr.us-east-1.amazonaws.com/container-registry:latest

# Push image
docker push <account-id>.dkr.ecr.us-east-1.amazonaws.com/container-registry:latest
```
...
# Run migrations
export DATABASE_URL="postgresql://admin:<password>@localhost:5432/container-registry"
...
aws ecs update-service \
  --cluster container-registry-cluster \
...
aws ecs wait services-stable \
  --cluster container-registry-cluster \
  --services container-registry-service
```
...
aws rds modify-db-instance \
  --db-instance-identifier container-registry-db \
...
docker build --no-cache -t container-registry:latest .
```
...
**Maintained by:** Container Registry Management Team
