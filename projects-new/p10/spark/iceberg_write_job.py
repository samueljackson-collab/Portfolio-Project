...
def write_bronze_table(
    ...
    df.writeTo(table_name).using("iceberg").tableProperty(
        "format-version", "2"
    ).tableProperty("write.metadata.compression-codec", "gzip").createOrReplace()
...
def append_to_silver_table(
    ...
    df.writeTo(target_table).using("iceberg").partitionedBy(
        "year", "month"
...
def main():
    ...
    parser.add_argument(
        "--mode", default="append", choices=["append", "overwrite", "merge"]
    )
