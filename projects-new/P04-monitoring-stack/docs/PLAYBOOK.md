# Operational Monitoring Stack - Implementation Playbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This playbook provides step-by-step instructions for implementing, deploying, and operating Operational Monitoring Stack from scratch.
...
cd portfolio/P04-monitoring-stack
```
...
../../scripts/bootstrap_remote_state.sh monitoring-stack us-east-1

# Output will show:
# S3 Bucket: monitoring-stack-tfstate-xxxxx
# DynamoDB Table: monitoring-stack-tfstate-lock
```
...
terraform {
  backend "s3" {
    bucket         = "monitoring-stack-tfstate-xxxxx"  # From bootstrap output
    key            = "monitoring-stack/terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "monitoring-stack-tfstate-lock"
    encrypt        = true
  }
}
```
...
```bash
# Check VPC
aws ec2 describe-vpcs --filters "Name=tag:Name,Values=monitoring-stack-vpc"

# Check RDS
aws rds describe-db-instances --db-instance-identifier monitoring-stack-db

# Check ECS cluster
aws ecs describe-clusters --clusters monitoring-stack-cluster
```
...
# Test locally
docker run -p 8080:8080 --env-file .env monitoring-stack:latest
...
# Tag image
docker tag monitoring-stack:latest <account-id>.dkr.ecr.us-east-1.amazonaws.com/monitoring-stack:latest

# Push image
docker push <account-id>.dkr.ecr.us-east-1.amazonaws.com/monitoring-stack:latest
```
...
# Run migrations
export DATABASE_URL="postgresql://admin:<password>@localhost:5432/monitoring-stack"
...
aws ecs update-service \
  --cluster monitoring-stack-cluster \
  --service monitoring-stack-service \
  --force-new-deployment

# Monitor deployment
aws ecs wait services-stable \
  --cluster monitoring-stack-cluster \
  --services monitoring-stack-service
```
...
aws rds modify-db-instance \
  --db-instance-identifier monitoring-stack-db \
...
docker build --no-cache -t monitoring-stack:latest .
```
...
**Maintained by:** Operational Monitoring Stack Team
