# Operational Monitoring Stack - Operations Runbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This runbook provides operational procedures, troubleshooting guides, and incident response workflows for Operational Monitoring Stack.
...
- **Service Name:** Operational Monitoring Stack
...
- **Grafana:** https://grafana.example.com/d/monitoring-stack
- **CloudWatch:** https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#dashboards:name=monitoring-stack
...
**Alert:** `monitoring-stack_high_error_rate`
...
```bash
# Check error logs
aws logs tail /aws/lambda/monitoring-stack --follow --filter '5xx'

# Check service health
kubectl get pods -n monitoring-stack
kubectl logs -n monitoring-stack <pod-name> --tail=100
```
...
**Alert:** `monitoring-stack_high_latency`
...
aws rds describe-db-instances --db-instance-identifier monitoring-stack-prod
...
kubectl top pods -n monitoring-stack
...
```markdown
# Incident Report: monitoring-stack Outage
...
aws s3 ls s3://backups-monitoring-stack/latest/
```
...
cat /etc/logrotate.d/monitoring-stack

# Test log rotation
logrotate -d /etc/logrotate.d/monitoring-stack
```
