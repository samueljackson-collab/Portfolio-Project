...
@dsl.component(
    ...
    packages_to_install=["pandas==2.0.3", "pyarrow==13.0.0", "great-expectations==0.17.15", "boto3==1.28.0"]
)
def data_ingestion(
    data_path: str,
    ...
    validator = context.sources.pandas_default.read_dataframe(df).get_validator(expectation_suite=suite)
...
def feature_engineering(
    ...
    df["sales_rolling_mean_7"] = df.groupby("store_id")["sales"].transform(lambda x: x.rolling(7, min_periods=1).mean())
...
@dsl.component(
    ...
    packages_to_install=["pandas==2.0.3", "scikit-learn==1.3.0", "mlflow==2.8.1", "boto3==1.28.0"]
)
def training(
    ...
    feature_cols = ["day_of_week", "month", "is_weekend", "sales_lag_7", "sales_rolling_mean_7"]
    X = df[feature_cols]
    y = df["sales"]
...
@dsl.component(
    base_image="python:3.10",
    packages_to_install=["mlflow==2.8.1", "boto3==1.28.0"]
)
def evaluation(
    ...
    if mae > baseline_mae_threshold:
        raise ValueError(f"Model MAE {mae:.2f} exceeds threshold {baseline_mae_threshold:.2f}. Failing pipeline.")

    print(f"Model passed evaluation gate. MAE: {mae:.2f} <= {baseline_mae_threshold:.2f}")
    return "PASS"
...
def training_pipeline(
    ...
    feature_task = feature_engineering(input_dataset=ingest_task.outputs["output_dataset"])
