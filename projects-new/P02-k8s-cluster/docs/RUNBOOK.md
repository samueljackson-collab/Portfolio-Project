# Kubernetes Cluster Management - Operations Runbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This runbook provides operational procedures, troubleshooting guides, and incident response workflows for Kubernetes Cluster Management.
...
- **Service Name:** Kubernetes Cluster Management
...
- **Grafana:** https://grafana.example.com/d/k8s-cluster
- **CloudWatch:** https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#dashboards:name=k8s-cluster
...
**Alert:** `k8s-cluster_high_error_rate`
...
```bash
# Check error logs
aws logs tail /aws/lambda/k8s-cluster --follow --filter '5xx'

# Check service health
kubectl get pods -n k8s-cluster
kubectl logs -n k8s-cluster <pod-name> --tail=100
```
...
**Alert:** `k8s-cluster_high_latency`
...
aws rds describe-db-instances --db-instance-identifier k8s-cluster-prod
...
kubectl top pods -n k8s-cluster
...
```markdown
# Incident Report: k8s-cluster Outage
...
aws s3 ls s3://backups-k8s-cluster/latest/
```
...
cat /etc/logrotate.d/k8s-cluster

# Test log rotation
logrotate -d /etc/logrotate.d/k8s-cluster
```
