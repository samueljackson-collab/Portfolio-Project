# Backup Verification System - Implementation Playbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This playbook provides step-by-step instructions for implementing, deploying, and operating Backup Verification System from scratch.
...
cd portfolio/P16-backup-verification
```
...
../../scripts/bootstrap_remote_state.sh backup-verification us-east-1

# Output will show:
# S3 Bucket: backup-verification-tfstate-xxxxx
# DynamoDB Table: backup-verification-tfstate-lock
```
...
terraform {
  backend "s3" {
    bucket         = "backup-verification-tfstate-xxxxx"  # From bootstrap output
    key            = "backup-verification/terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "backup-verification-tfstate-lock"
    encrypt        = true
  }
}
```
...
```bash
# Check VPC
aws ec2 describe-vpcs --filters "Name=tag:Name,Values=backup-verification-vpc"

# Check RDS
aws rds describe-db-instances --db-instance-identifier backup-verification-db

# Check ECS cluster
aws ecs describe-clusters --clusters backup-verification-cluster
```
...
```bash
# Build image
docker build -t backup-verification:latest .

# Test locally
docker run -p 8080:8080 --env-file .env backup-verification:latest
...
# Tag image
docker tag backup-verification:latest <account-id>.dkr.ecr.us-east-1.amazonaws.com/backup-verification:latest

# Push image
docker push <account-id>.dkr.ecr.us-east-1.amazonaws.com/backup-verification:latest
```
...
# Run migrations
export DATABASE_URL="postgresql://admin:<password>@localhost:5432/backup-verification"
...
aws ecs update-service \
  ...
  --service backup-verification-service \
...
aws rds modify-db-instance \
  --db-instance-identifier backup-verification-db \
...
docker build --no-cache -t backup-verification:latest .
```
...
**Maintained by:** Backup Verification System Team
