# Security Compliance Automation - Implementation Playbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This playbook provides step-by-step instructions for implementing, deploying, and operating Security Compliance Automation from scratch.
...
cd portfolio/P07-security-compliance
```
...
../../scripts/bootstrap_remote_state.sh security-compliance us-east-1

# Output will show:
# S3 Bucket: security-compliance-tfstate-xxxxx
# DynamoDB Table: security-compliance-tfstate-lock
```
...
terraform {
  backend "s3" {
    bucket         = "security-compliance-tfstate-xxxxx"  # From bootstrap output
    key            = "security-compliance/terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "security-compliance-tfstate-lock"
    encrypt        = true
  }
}
```
...
```bash
# Check VPC
aws ec2 describe-vpcs --filters "Name=tag:Name,Values=security-compliance-vpc"

# Check RDS
aws rds describe-db-instances --db-instance-identifier security-compliance-db

# Check ECS cluster
aws ecs describe-clusters --clusters security-compliance-cluster
```
...
```bash
# Build image
docker build -t security-compliance:latest .

# Test locally
docker run -p 8080:8080 --env-file .env security-compliance:latest
...
# Tag image
docker tag security-compliance:latest <account-id>.dkr.ecr.us-east-1.amazonaws.com/security-compliance:latest

# Push image
docker push <account-id>.dkr.ecr.us-east-1.amazonaws.com/security-compliance:latest
```
...
# Run migrations
export DATABASE_URL="postgresql://admin:<password>@localhost:5432/security-compliance"
...
aws ecs update-service \
  --cluster security-compliance-cluster \
...
aws ecs wait services-stable \
  --cluster security-compliance-cluster \
  --services security-compliance-service
```
...
aws rds modify-db-instance \
  --db-instance-identifier security-compliance-db \
...
docker build --no-cache -t security-compliance:latest .
```
...
**Maintained by:** Security Compliance Automation Team
