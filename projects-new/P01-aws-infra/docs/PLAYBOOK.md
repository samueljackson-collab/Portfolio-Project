# AWS Infrastructure Automation - Implementation Playbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This playbook provides step-by-step instructions for implementing, deploying, and operating AWS Infrastructure Automation from scratch.
...
../../scripts/bootstrap_remote_state.sh aws-infra us-east-1

# Output will show:
# S3 Bucket: aws-infra-tfstate-xxxxx
# DynamoDB Table: aws-infra-tfstate-lock
```
...
terraform {
  backend "s3" {
    bucket         = "aws-infra-tfstate-xxxxx"  # From bootstrap output
...
```bash
# Check VPC
aws ec2 describe-vpcs --filters "Name=tag:Name,Values=aws-infra-vpc"

# Check RDS
aws rds describe-db-instances --db-instance-identifier aws-infra-db

# Check ECS cluster
aws ecs describe-clusters --clusters aws-infra-cluster
```
...
```bash
# Build image
docker build -t aws-infra:latest .

# Test locally
docker run -p 8080:8080 --env-file .env aws-infra:latest
...
# Tag image
docker tag aws-infra:latest <account-id>.dkr.ecr.us-east-1.amazonaws.com/aws-infra:latest

# Push image
docker push <account-id>.dkr.ecr.us-east-1.amazonaws.com/aws-infra:latest
```
...
# Run migrations
export DATABASE_URL="postgresql://admin:<password>@localhost:5432/aws-infra"
...
aws ecs update-service \
  --cluster aws-infra-cluster \
  --service aws-infra-service \
  --force-new-deployment

# Monitor deployment
aws ecs wait services-stable \
  --cluster aws-infra-cluster \
  --services aws-infra-service
```
...
aws rds modify-db-instance \
  --db-instance-identifier aws-infra-db \
...
docker build --no-cache -t aws-infra:latest .
```
...
**Maintained by:** AWS Infrastructure Automation Team
