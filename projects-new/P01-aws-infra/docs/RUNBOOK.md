# AWS Infrastructure Automation - Operations Runbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This runbook provides operational procedures, troubleshooting guides, and incident response workflows for AWS Infrastructure Automation.
...
- **Service Name:** AWS Infrastructure Automation
...
- **Grafana:** https://grafana.example.com/d/aws-infra
- **CloudWatch:** https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#dashboards:name=aws-infra
...
**Alert:** `aws-infra_high_error_rate`
...
```bash
# Check error logs
aws logs tail /aws/lambda/aws-infra --follow --filter '5xx'

# Check service health
kubectl get pods -n aws-infra
kubectl logs -n aws-infra <pod-name> --tail=100
```
...
aws rds describe-db-instances --db-instance-identifier aws-infra-prod
...
```markdown
# Incident Report: aws-infra Outage
...
aws s3 ls s3://backups-aws-infra/latest/
```
...
cat /etc/logrotate.d/aws-infra

# Test log rotation
logrotate -d /etc/logrotate.d/aws-infra
```
