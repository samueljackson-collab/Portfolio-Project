# API Gateway Configuration - Operations Runbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This runbook provides operational procedures, troubleshooting guides, and incident response workflows for API Gateway Configuration.
...
- **Service Name:** API Gateway Configuration
...
- **CloudWatch:** https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#dashboards:name=api-gateway
...
**Alert:** `api-gateway_high_error_rate`
...
```bash
# Check error logs
aws logs tail /aws/lambda/api-gateway --follow --filter '5xx'

# Check service health
kubectl get pods -n api-gateway
kubectl logs -n api-gateway <pod-name> --tail=100
```
...
**Alert:** `api-gateway_high_latency`
...
aws rds describe-db-instances --db-instance-identifier api-gateway-prod
...
aws s3 ls s3://backups-api-gateway/latest/
```
...
cat /etc/logrotate.d/api-gateway

# Test log rotation
logrotate -d /etc/logrotate.d/api-gateway
```
