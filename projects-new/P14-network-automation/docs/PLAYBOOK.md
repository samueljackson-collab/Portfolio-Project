# Network Configuration Automation - Implementation Playbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This playbook provides step-by-step instructions for implementing, deploying, and operating Network Configuration Automation from scratch.
...
cd portfolio/P14-network-automation
```
...
../../scripts/bootstrap_remote_state.sh network-automation us-east-1

# Output will show:
# S3 Bucket: network-automation-tfstate-xxxxx
# DynamoDB Table: network-automation-tfstate-lock
```
...
terraform {
  backend "s3" {
    bucket         = "network-automation-tfstate-xxxxx"  # From bootstrap output
    key            = "network-automation/terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "network-automation-tfstate-lock"
    encrypt        = true
  }
}
```
...
```bash
# Check VPC
aws ec2 describe-vpcs --filters "Name=tag:Name,Values=network-automation-vpc"

# Check RDS
aws rds describe-db-instances --db-instance-identifier network-automation-db

# Check ECS cluster
aws ecs describe-clusters --clusters network-automation-cluster
```
...
# Test locally
docker run -p 8080:8080 --env-file .env network-automation:latest
...
# Tag image
docker tag network-automation:latest <account-id>.dkr.ecr.us-east-1.amazonaws.com/network-automation:latest

# Push image
docker push <account-id>.dkr.ecr.us-east-1.amazonaws.com/network-automation:latest
```
...
# Run migrations
export DATABASE_URL="postgresql://admin:<password>@localhost:5432/network-automation"
...
aws ecs update-service \
  --cluster network-automation-cluster \
  --service network-automation-service \
  --force-new-deployment

# Monitor deployment
aws ecs wait services-stable \
  --cluster network-automation-cluster \
  --services network-automation-service
```
...
aws rds modify-db-instance \
  --db-instance-identifier network-automation-db \
...
docker build --no-cache -t network-automation:latest .
```
...
**Maintained by:** Network Configuration Automation Team
