# Log Aggregation System - Implementation Playbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This playbook provides step-by-step instructions for implementing, deploying, and operating Log Aggregation System from scratch.
...
cd portfolio/P10-log-aggregation
```
...
../../scripts/bootstrap_remote_state.sh log-aggregation us-east-1

# Output will show:
# S3 Bucket: log-aggregation-tfstate-xxxxx
# DynamoDB Table: log-aggregation-tfstate-lock
```
...
terraform {
  backend "s3" {
    bucket         = "log-aggregation-tfstate-xxxxx"  # From bootstrap output
    key            = "log-aggregation/terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "log-aggregation-tfstate-lock"
    encrypt        = true
  }
}
```
...
```bash
# Check VPC
aws ec2 describe-vpcs --filters "Name=tag:Name,Values=log-aggregation-vpc"

# Check RDS
aws rds describe-db-instances --db-instance-identifier log-aggregation-db

# Check ECS cluster
aws ecs describe-clusters --clusters log-aggregation-cluster
```
...
```bash
# Build image
docker build -t log-aggregation:latest .

# Test locally
docker run -p 8080:8080 --env-file .env log-aggregation:latest
...
# Tag image
docker tag log-aggregation:latest <account-id>.dkr.ecr.us-east-1.amazonaws.com/log-aggregation:latest

# Push image
docker push <account-id>.dkr.ecr.us-east-1.amazonaws.com/log-aggregation:latest
```
...
# Run migrations
export DATABASE_URL="postgresql://admin:<password>@localhost:5432/log-aggregation"
...
aws ecs update-service \
  --cluster log-aggregation-cluster \
...
aws ecs wait services-stable \
  --cluster log-aggregation-cluster \
  --services log-aggregation-service
```
...
aws rds modify-db-instance \
  --db-instance-identifier log-aggregation-db \
...
docker build --no-cache -t log-aggregation:latest .
```
...
**Maintained by:** Log Aggregation System Team
