# Database Performance Optimization - Implementation Playbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This playbook provides step-by-step instructions for implementing, deploying, and operating Database Performance Optimization from scratch.
...
cd portfolio/P05-db-optimization
```
...
../../scripts/bootstrap_remote_state.sh db-optimization us-east-1

# Output will show:
# S3 Bucket: db-optimization-tfstate-xxxxx
# DynamoDB Table: db-optimization-tfstate-lock
```
...
terraform {
  backend "s3" {
    bucket         = "db-optimization-tfstate-xxxxx"  # From bootstrap output
    key            = "db-optimization/terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "db-optimization-tfstate-lock"
    encrypt        = true
  }
}
```
...
```bash
# Check VPC
aws ec2 describe-vpcs --filters "Name=tag:Name,Values=db-optimization-vpc"

# Check RDS
aws rds describe-db-instances --db-instance-identifier db-optimization-db

# Check ECS cluster
aws ecs describe-clusters --clusters db-optimization-cluster
```
...
```bash
# Build image
docker build -t db-optimization:latest .

# Test locally
docker run -p 8080:8080 --env-file .env db-optimization:latest
...
# Tag image
docker tag db-optimization:latest <account-id>.dkr.ecr.us-east-1.amazonaws.com/db-optimization:latest

# Push image
docker push <account-id>.dkr.ecr.us-east-1.amazonaws.com/db-optimization:latest
```
...
# Run migrations
export DATABASE_URL="postgresql://admin:<password>@localhost:5432/db-optimization"
...
aws ecs update-service \
  --cluster db-optimization-cluster \
  --service db-optimization-service \
  --force-new-deployment

# Monitor deployment
aws ecs wait services-stable \
  --cluster db-optimization-cluster \
  --services db-optimization-service
```
...
aws rds modify-db-instance \
  --db-instance-identifier db-optimization-db \
...
docker build --no-cache -t db-optimization:latest .
```
...
**Maintained by:** Database Performance Optimization Team
