# Database Performance Optimization - Operations Runbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This runbook provides operational procedures, troubleshooting guides, and incident response workflows for Database Performance Optimization.
...
- **Service Name:** Database Performance Optimization
...
- **Grafana:** https://grafana.example.com/d/db-optimization
- **CloudWatch:** https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#dashboards:name=db-optimization
...
**Alert:** `db-optimization_high_error_rate`
...
```bash
# Check error logs
aws logs tail /aws/lambda/db-optimization --follow --filter '5xx'

# Check service health
kubectl get pods -n db-optimization
kubectl logs -n db-optimization <pod-name> --tail=100
```
...
**Alert:** `db-optimization_high_latency`
...
aws rds describe-db-instances --db-instance-identifier db-optimization-prod
...
kubectl top pods -n db-optimization
...
```markdown
# Incident Report: db-optimization Outage
...
aws s3 ls s3://backups-db-optimization/latest/
```
...
cat /etc/logrotate.d/db-optimization

# Test log rotation
logrotate -d /etc/logrotate.d/db-optimization
```
