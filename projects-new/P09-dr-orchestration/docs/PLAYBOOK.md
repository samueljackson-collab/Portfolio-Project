# Disaster Recovery Orchestration - Implementation Playbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This playbook provides step-by-step instructions for implementing, deploying, and operating Disaster Recovery Orchestration from scratch.
...
cd portfolio/P09-dr-orchestration
```
...
../../scripts/bootstrap_remote_state.sh dr-orchestration us-east-1

# Output will show:
# S3 Bucket: dr-orchestration-tfstate-xxxxx
# DynamoDB Table: dr-orchestration-tfstate-lock
```
...
terraform {
  backend "s3" {
    bucket         = "dr-orchestration-tfstate-xxxxx"  # From bootstrap output
    key            = "dr-orchestration/terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "dr-orchestration-tfstate-lock"
    encrypt        = true
  }
}
```
...
```bash
# Check VPC
aws ec2 describe-vpcs --filters "Name=tag:Name,Values=dr-orchestration-vpc"

# Check RDS
aws rds describe-db-instances --db-instance-identifier dr-orchestration-db

# Check ECS cluster
aws ecs describe-clusters --clusters dr-orchestration-cluster
```
...
```bash
# Build image
docker build -t dr-orchestration:latest .

# Test locally
docker run -p 8080:8080 --env-file .env dr-orchestration:latest
...
# Tag image
docker tag dr-orchestration:latest <account-id>.dkr.ecr.us-east-1.amazonaws.com/dr-orchestration:latest

# Push image
docker push <account-id>.dkr.ecr.us-east-1.amazonaws.com/dr-orchestration:latest
```
...
# Run migrations
export DATABASE_URL="postgresql://admin:<password>@localhost:5432/dr-orchestration"
...
aws ecs update-service \
  --cluster dr-orchestration-cluster \
  --service dr-orchestration-service \
  --force-new-deployment

# Monitor deployment
aws ecs wait services-stable \
  --cluster dr-orchestration-cluster \
  --services dr-orchestration-service
```
...
aws rds modify-db-instance \
  --db-instance-identifier dr-orchestration-db \
...
docker build --no-cache -t dr-orchestration:latest .
```
...
**Maintained by:** Disaster Recovery Orchestration Team
