    ...
    try:
        ...
        try:
            response = s3_client.get_object(
                Bucket=bucket, Key=key, VersionId=version_id if version_id else None
            )
        ...
        try:
            ...
            update_metadata_status(
                execution_id, timestamp, "validation_failed", error_msg
            )
            ...
            try:
                ...
                if event_time > datetime.now(timezone.utc):
                    validation_errors.append(
                        f"Timestamp in future: {data['timestamp']}"
                    )
            except ValueError:
                validation_errors.append(
    ...
    try:
        ...
        sqs_client.send_message(
            ...
            MessageAttributes={
                ...
                "ExecutionId": {
                    "StringValue": event["execution_id"],
                    "DataType": "String",
                },
            },
        )
...
def update_metadata_status(
    execution_id: str, timestamp: int, status: str, error_message: Optional[str]
) -> None:
    """
    ...
    try:
        update_expression = (
            "SET #status = :status, validation_timestamp = :validation_timestamp"
        )
        expression_values = {
            ":status": status,
            ":validation_timestamp": datetime.now(timezone.utc).isoformat(),
