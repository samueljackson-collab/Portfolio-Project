# Service Mesh Implementation - Operations Runbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This runbook provides operational procedures, troubleshooting guides, and incident response workflows for Service Mesh Implementation.
...
- **Service Name:** Service Mesh Implementation
...
- **Grafana:** https://grafana.example.com/d/service-mesh
- **CloudWatch:** https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#dashboards:name=service-mesh
...
**Alert:** `service-mesh_high_error_rate`
...
```bash
# Check error logs
aws logs tail /aws/lambda/service-mesh --follow --filter '5xx'

# Check service health
kubectl get pods -n service-mesh
kubectl logs -n service-mesh <pod-name> --tail=100
```
...
**Alert:** `service-mesh_high_latency`
...
aws rds describe-db-instances --db-instance-identifier service-mesh-prod
...
kubectl top pods -n service-mesh
...
```markdown
# Incident Report: service-mesh Outage
...
aws s3 ls s3://backups-service-mesh/latest/
```
...
cat /etc/logrotate.d/service-mesh

# Test log rotation
logrotate -d /etc/logrotate.d/service-mesh
```
