# Observability Dashboard - Operations Runbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This runbook provides operational procedures, troubleshooting guides, and incident response workflows for Observability Dashboard.
...
- **Service Name:** Observability Dashboard
...
# 2. Pull latest code
cd /opt/observability-dashboard
git fetch origin
...
- **Grafana:** https://grafana.example.com/d/observability-dashboard
- **CloudWatch:** https://console.aws.amazon.com/cloudwatch/home?region=us-east-1#dashboards:name=observability-dashboard
...
**Alert:** `observability-dashboard_high_error_rate`
...
```bash
# Check error logs
aws logs tail /aws/lambda/observability-dashboard --follow --filter '5xx'

# Check service health
kubectl get pods -n observability-dashboard
kubectl logs -n observability-dashboard <pod-name> --tail=100
```
...
**Alert:** `observability-dashboard_high_latency`
...
aws rds describe-db-instances --db-instance-identifier observability-dashboard-prod
...
kubectl top pods -n observability-dashboard
...
```markdown
# Incident Report: observability-dashboard Outage
...
aws s3 ls s3://backups-observability-dashboard/latest/
```
...
cat /etc/logrotate.d/observability-dashboard

# Test log rotation
logrotate -d /etc/logrotate.d/observability-dashboard
```
