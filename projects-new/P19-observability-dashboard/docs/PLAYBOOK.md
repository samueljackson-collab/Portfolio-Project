# Observability Dashboard - Implementation Playbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This playbook provides step-by-step instructions for implementing, deploying, and operating Observability Dashboard from scratch.
...
cd portfolio/P19-observability-dashboard
```
...
DB_HOST=localhost
DB_PORT=5432
DB_NAME=observability-dashboard
DB_USER=admin
...
../../scripts/bootstrap_remote_state.sh observability-dashboard us-east-1

# Output will show:
# S3 Bucket: observability-dashboard-tfstate-xxxxx
# DynamoDB Table: observability-dashboard-tfstate-lock
```
...
terraform {
  backend "s3" {
    bucket         = "observability-dashboard-tfstate-xxxxx"  # From bootstrap output
    key            = "observability-dashboard/terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "observability-dashboard-tfstate-lock"
    encrypt        = true
  }
}
```
...
```bash
# Check VPC
aws ec2 describe-vpcs --filters "Name=tag:Name,Values=observability-dashboard-vpc"

# Check RDS
aws rds describe-db-instances --db-instance-identifier observability-dashboard-db

# Check ECS cluster
aws ecs describe-clusters --clusters observability-dashboard-cluster
```
...
# Test locally
docker run -p 8080:8080 --env-file .env observability-dashboard:latest
...
# Tag image
docker tag observability-dashboard:latest <account-id>.dkr.ecr.us-east-1.amazonaws.com/observability-dashboard:latest

# Push image
docker push <account-id>.dkr.ecr.us-east-1.amazonaws.com/observability-dashboard:latest
```
...
# Run migrations
export DATABASE_URL="postgresql://admin:<password>@localhost:5432/observability-dashboard"
...
aws ecs update-service \
  --cluster observability-dashboard-cluster \
  --service observability-dashboard-service \
  --force-new-deployment

# Monitor deployment
aws ecs wait services-stable \
  --cluster observability-dashboard-cluster \
  --services observability-dashboard-service
```
...
aws rds modify-db-instance \
  --db-instance-identifier observability-dashboard-db \
...
docker build --no-cache -t observability-dashboard:latest .
```
...
**Maintained by:** Observability Dashboard Team
