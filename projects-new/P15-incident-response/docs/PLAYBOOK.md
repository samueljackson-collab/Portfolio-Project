# Incident Response Automation - Implementation Playbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This playbook provides step-by-step instructions for implementing, deploying, and operating Incident Response Automation from scratch.
...
cd portfolio/P15-incident-response
```
...
../../scripts/bootstrap_remote_state.sh incident-response us-east-1

# Output will show:
# S3 Bucket: incident-response-tfstate-xxxxx
# DynamoDB Table: incident-response-tfstate-lock
```
...
terraform {
  backend "s3" {
    bucket         = "incident-response-tfstate-xxxxx"  # From bootstrap output
    key            = "incident-response/terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "incident-response-tfstate-lock"
    encrypt        = true
  }
}
```
...
```bash
# Check VPC
aws ec2 describe-vpcs --filters "Name=tag:Name,Values=incident-response-vpc"

# Check RDS
aws rds describe-db-instances --db-instance-identifier incident-response-db

# Check ECS cluster
aws ecs describe-clusters --clusters incident-response-cluster
```
...
# Test locally
docker run -p 8080:8080 --env-file .env incident-response:latest
...
# Tag image
docker tag incident-response:latest <account-id>.dkr.ecr.us-east-1.amazonaws.com/incident-response:latest

# Push image
docker push <account-id>.dkr.ecr.us-east-1.amazonaws.com/incident-response:latest
```
...
# Run migrations
export DATABASE_URL="postgresql://admin:<password>@localhost:5432/incident-response"
...
aws ecs update-service \
  ...
  --service incident-response-service \
...
aws rds modify-db-instance \
  --db-instance-identifier incident-response-db \
...
docker build --no-cache -t incident-response:latest .
```
...
**Maintained by:** Incident Response Automation Team
