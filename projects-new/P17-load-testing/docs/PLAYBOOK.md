# Performance Load Testing - Implementation Playbook

**Version:** 1.0.0 | **Last Updated:** 2025-11-10

## Overview

This playbook provides step-by-step instructions for implementing, deploying, and operating Performance Load Testing from scratch.
...
../../scripts/bootstrap_remote_state.sh load-testing us-east-1

# Output will show:
# S3 Bucket: load-testing-tfstate-xxxxx
# DynamoDB Table: load-testing-tfstate-lock
```
...
terraform {
  backend "s3" {
    bucket         = "load-testing-tfstate-xxxxx"  # From bootstrap output
    key            = "load-testing/terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "load-testing-tfstate-lock"
    encrypt        = true
  }
}
```
...
```bash
# Check VPC
aws ec2 describe-vpcs --filters "Name=tag:Name,Values=load-testing-vpc"

# Check RDS
aws rds describe-db-instances --db-instance-identifier load-testing-db

# Check ECS cluster
aws ecs describe-clusters --clusters load-testing-cluster
```
...
```bash
# Build image
docker build -t load-testing:latest .

# Test locally
docker run -p 8080:8080 --env-file .env load-testing:latest
...
# Tag image
docker tag load-testing:latest <account-id>.dkr.ecr.us-east-1.amazonaws.com/load-testing:latest

# Push image
docker push <account-id>.dkr.ecr.us-east-1.amazonaws.com/load-testing:latest
```
...
# Run migrations
export DATABASE_URL="postgresql://admin:<password>@localhost:5432/load-testing"
...
aws ecs update-service \
  --cluster load-testing-cluster \
  --service load-testing-service \
  --force-new-deployment

# Monitor deployment
aws ecs wait services-stable \
  --cluster load-testing-cluster \
  --services load-testing-service
```
...
aws rds modify-db-instance \
  --db-instance-identifier load-testing-db \
...
docker build --no-cache -t load-testing:latest .
```
...
**Maintained by:** Performance Load Testing Team
