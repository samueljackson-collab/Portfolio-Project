...
description: - Verify `PGHOST`, `PGUSER`, and `PGPASSWORD` are set. - Ensure the `BACKUP_ROOT` directory is writable. - Check `pg_dump` version compatibility with server. - Validate `archive_mode=on` and correct `
tags: [analytics, data-engineering, documentation, pipeline, portfolio]
path: portfolio/2-database-migration/troubleshooting
created: 2026-03-08T22:19:13.257013+00:00
