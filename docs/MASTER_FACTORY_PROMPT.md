...
Everything above is mandatory—no sections may be skipped or abbreviated. Produce the full content for each element rather than instructions or TODOs.
...
- Do not return prompts that would generate these assets later; deliver the finalized code, data, diagrams, and narrative directly.
```
