...
class TestPrometheusConfig:
    ...
    def test_prometheus_has_required_sections(self):
        ...
        assert "scrape_configs" in config or "scrape_interval" in config.get(
            "global", {}
        )
    ...
    def test_prometheus_global_config(self):
        ...
        assert (
            "evaluation_interval" in global_config or "scrape_interval" in global_config
        )
...
class TestLokiConfig:
    ...
    def test_loki_config_valid_yaml(self):
        ...
        config_path = (
            BASE_PATH / "projects/01-sde-devops/PRJ-SDE-002/assets/loki/loki-config.yml"
        )
        assert config_path.exists(), f"Config not found at {config_path}"
    ...
    def test_loki_has_server_config(self):
        ...
        assert (
            "server" in config or "http_listen_port" in config.get("server", {}) or True
        )
...
class TestArgoCDApplication:
    ...
    def test_argocd_application_valid_yaml(self):
        ...
        config_path = (
            BASE_PATH
            / "enterprise-portfolio/kubernetes/manifests/argocd/application.yaml"
        )
        assert config_path.exists(), f"Config not found at {config_path}"
