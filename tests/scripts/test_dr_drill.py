...
SCRIPT_PATH = Path(__file__).parent.parent.parent / "projects/p01-aws-infra/scripts/dr-drill.sh"
...
class TestDRDrillBasicFunctionality:
    ...
    def test_help_flag_displays_usage(self):
        ...
        result = subprocess.run([str(SCRIPT_PATH), "report", "--help"], capture_output=True, text=True)  # noqa: S603, S607
        assert "Usage:" in result.stdout or "Usage:" in result.stderr
    
    def test_no_command_shows_usage(self):
        """Test that running without command shows usage"""
        result = subprocess.run([str(SCRIPT_PATH)], capture_output=True, text=True)  # noqa: S603, S607
        assert result.returncode == 1
        assert "Usage:" in result.stdout or "Usage:" in result.stderr
    
    def test_unknown_command_shows_error(self):
        """Test that unknown command produces error"""
        result = subprocess.run([str(SCRIPT_PATH), "invalid-command"], capture_output=True, text=True)  # noqa: S603, S607
        assert result.returncode == 1
...
class TestFailoverCommand:
    ...
    def test_failover_requires_db_instance(self):
        ...
        assert "test-failover-db" in result.stdout or result.stderr or result.returncode != 1
...
class TestEnvironmentVariables:
    ...
    def test_dr_drill_terraform_dir_env(self):
        ...
        env = os.environ.copy()
        env["DR_DRILL_TERRAFORM_DIR"] = os.path.join(tempfile.gettempdir(), "test-terraform")
...
class TestTerraformIntegration:
    ...
    def test_terraform_dir_flag_override(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            result = subprocess.run(
                [str(SCRIPT_PATH), "report", "--terraform-dir", tmpdir, "--db-instance-id", "test-db"],
                capture_output=True,
                text=True
            )  # noqa: S603, S607
