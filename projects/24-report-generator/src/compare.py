...
class HistoricalComparison:
    ...
    def compare_snapshots(
        ...
        return {
            ...
            'time_delta_days': self._calculate_time_delta(
                current.get('snapshot_timestamp'),
                previous.get('snapshot_timestamp')
            ),
    ...
    def _generate_insights(
        ...
        for comp in comparisons:
            if comp.trend == "up" and comp.delta_percentage and comp.delta_percentage > 10:
                insights.append(
        ...
        for comp in comparisons:
            if comp.trend == "down" and comp.delta_percentage and comp.delta_percentage < -10:
                insights.append(
                    f"⚠️ Notable decrease in {comp.metric}: {comp.interpretation}"
                )

        # Check for high completion
        completion_comp = next((c for c in comparisons if 'Completion' in c.metric), None)
        if completion_comp and completion_comp.current_value >= 90:
            insights.append(
                "🎉 Portfolio has reached high completion rate (90%+)"
            )
...
def main():
    ...
    parser.add_argument(
        '--data-file',
        type=str,
        help='Current data file (for save/compare)'
    )
    parser.add_argument(
        '--days-back',
        type=int,
        default=7,
        help='Days to look back for comparison'
    )
    parser.add_argument(
        '--metric',
        type=str,
        help='Metric to track (for trend)'
    )
    ...
    if args.action == 'save':
        ...
        previous_data = comparator.get_snapshot_before(
            datetime.now(),
            args.days_back
        )
        ...
        print(f"Comparing: {comparison['previous_timestamp']} → {comparison['current_timestamp']}")
        ...
        for comp in comparison['metric_comparisons']:
            trend_icon = "↑" if comp['trend'] == "up" else "↓" if comp['trend'] == "down" else "→"
            print(f"  {trend_icon} {comp['metric']}: {comp['previous_value']} → {comp['current_value']} ({comp['interpretation']})")
