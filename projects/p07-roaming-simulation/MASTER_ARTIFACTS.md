...
Prompt: *Generate Terraform that provisions a least-privilege VPC, subnets, route tables, security groups, and managed identities for the Roaming Simulation Platform. Enforce tagging standards, enable flow logs, and include policy-as-code checks for network boundaries.*
...
Prompt: *Create a backend microservice for Roaming Simulation Platform using a clean architecture pattern. Include REST and gRPC surfaces, OpenAPI/Protobuf contracts, feature-flag toggles, idempotent handlers, and structured logging. Add automated integration tests that run against ephemeral test containers.*
...
Prompt: *Build a frontend console that visualizes network roaming, mobility policy testing, telecom-grade simulation. Use accessible design, server-driven UI schemas, role-based access control, and real-time updates via WebSockets or SSE. Include Cypress tests for smoke, regression, and accessibility.*
...
Prompt: *Implement observability for Roaming Simulation Platform capturing RED/USE metrics, trace sampling rules, structured logs with correlation IDs, and SLO dashboards. Provide runbook-driven alert rules with explicit owners and escalation policies.*
...
Scope & Goals: Define datasets, schemas, retention needs, and data sovereignty requirements for the Roaming Simulation Platform. Clarify cutover windows, rollback strategies, and success criteria (row counts, checksums, synthetic transactions).
...
**Narrative**: Describe a realistic scenario (e.g., high load, partial outage, vendor dependency failure) and explain how the Roaming Simulation Platform architecture responds. Include detection paths, decision trees, and the rollback or feature-flag strategy that protects end users.
