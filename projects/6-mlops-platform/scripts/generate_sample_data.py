...
def generate_drifted_dataset(base_df, drift_strength=0.3, drift_features=5):
    ...
    df_drifted["timestamp"] = pd.date_range(
        start="2024-06-01", periods=len(df_drifted), freq="5min"
    )
...
def generate_all_datasets():
    ...
    metadata = {
        "classification": {
            ...
            "n_features": len(
                [c for c in classification_df.columns if c.startswith("feature_")]
            ),
        ...
        "regression": {
            ...
            "n_features": len(
                [c for c in regression_df.columns if c.startswith("feature_")]
            ),
            "task": "regression",
