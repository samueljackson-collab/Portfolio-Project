...
def check_s3_bucket_logging():
    ...
    if errors:
        print(
            f"  ℹ️  Encountered {len(errors)} errors during bucket checks (may indicate permission issues)"
        )
    ...
    if len(issues) == 0:
        ...
        print(
            f"✗ CIS 2.6: S3 bucket logging disabled for {len(issues)} buckets: {', '.join(issues[:5])}"
        )
        if len(issues) > 5:
