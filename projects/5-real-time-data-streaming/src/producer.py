...
class EventProducer:
    ...
    def __init__(self, bootstrap_servers: List[str] = None, topic: str = "user-events"):
        """
        ...
        logger.info(
            f"Producer initialized for topic '{topic}' on {self.bootstrap_servers}"
        )
    ...
    def simulate_high_volume_stream(
        self, duration_seconds: int = 60, events_per_second: int = 100
    ):
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--topic", default="user-events", help="Kafka topic to publish to"
    )
    parser.add_argument(
        "--count", type=int, default=1000, help="Number of events to generate"
    )
    parser.add_argument(
        "--delay", type=float, default=0.1, help="Delay between events in seconds"
    )
    parser.add_argument(
        "--high-volume", action="store_true", help="Run high-volume simulation"
    )
    parser.add_argument(
        "--duration",
        type=int,
    ...
    with EventProducer(
        bootstrap_servers=bootstrap_servers, topic=args.topic
    ) as producer:
        if args.high_volume:
            producer.simulate_high_volume_stream(
                duration_seconds=args.duration, events_per_second=args.rate
            )
        else:
