...
class TestNativeMessagingHost(unittest.TestCase):
    ...
    def test_send_message_error_handling(self):
        ...
        with patch(
            "sys.stdout",
            Mock(buffer=Mock(write=Mock(side_effect=Exception("Write failed")))),
        ):
...
class TestNativeHostIntegration(unittest.TestCase):
    ...
    def test_large_message_handling(self):
        ...
        large_message = {
            "type": "tabs_update",
            "tabs": [
                {"id": str(i), "url": f"https://example{i}.com"} for i in range(10000)
            ],
        }
...
class TestNativeHostEdgeCases(unittest.TestCase):
    ...
    def test_message_with_null_values(self):
        ...
        null_message = {"type": "test", "value": None, "nested": {"null": None}}
...
for attr_name in dir(TestNativeMessagingHost):
    if attr_name.startswith("test_") and asyncio.iscoroutinefunction(
        getattr(TestNativeMessagingHost, attr_name)
    ):
        setattr(
            TestNativeMessagingHost,
            attr_name,
            async_test(getattr(TestNativeMessagingHost, attr_name)),
        )
