...
class PolicyValidator:
    ...
    def validate_policy(self, policy: Dict, source: str = "policy") -> Tuple[bool, List[str]]:
        """
    ...
    def _check_wildcards(self, statement: Dict, prefix: str):
        ...
        if '*' in actions:
            if statement.get('Effect') == 'Allow':
                ...
                self.info.append(f"{prefix}: Uses Deny with wildcard (acceptable pattern)")
...
def main():
    ...
    parser.add_argument(
        'policy_files',
        nargs='+',
        help='Policy JSON files to validate'
    )
    parser.add_argument(
        '--strict',
        action='store_true',
    ...
    for policy_file in args.policy_files:
        ...
        issue_count = len([m for m in messages if 'ERROR' in m or m in validator.issues])
