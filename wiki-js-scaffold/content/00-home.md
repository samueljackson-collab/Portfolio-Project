# GitHub Fundamentals Wiki Home

Welcome to the GitHub Fundamentals Wiki.js hub. This space turns the course into a searchable, production-ready knowledge base with clear navigation, guided learning paths, and hands-on project references.

## 🚀 Start Here

- **New to Git & GitHub?** Begin with the [Setup & Fundamentals overview](/setup-fundamentals).
- **Already using Git?** Jump to [Git Fundamentals](/git-fundamentals) or [Documentation](/documentation).
- **Building a portfolio?** Head straight to [Real-World Projects](/real-world-projects).

## 🧭 Quick Navigation

| Section | Focus | Best for |
| --- | --- | --- |
| [Setup & Fundamentals](./01-setup-fundamentals/00-index.md) | Installations, accounts, and core setup | First-time learners |
| [Git Fundamentals](./02-git-fundamentals/00-index.md) | Daily workflow commands and concepts | Building confidence |
| [Documentation](./03-documentation/00-index.md) | README, .gitignore, licensing | Polishing repositories |
| [Real-World Projects](./04-real-world-projects/00-index.md) | Portfolio and collaboration flows | Practice & delivery |
| [GitHub Platform](./05-github-platform/00-index.md) | Issues, Actions, security | Platform power users |
| [Advanced Topics](./06-advanced-topics/00-index.md) | Rebase, advanced workflows | Experienced users |
| [Next Steps](./07-next-steps/00-index.md) | Career growth, open source | Long-term growth |

## ✅ What You’ll Gain

- A structured learning path with **clear progression** and **next-step guidance**.
- A Wiki.js layout that supports **fast search**, **section overviews**, and **project-ready workflows**.
- Practical deliverables you can reuse for interviews, onboarding, or portfolio reviews.

## 🔎 Recommended Learning Paths

### Beginner Track
1. [Setup & Fundamentals](/setup-fundamentals)
2. [Git Fundamentals](/git-fundamentals)
3. [Documentation](/documentation)

### Portfolio Track
1. [Real-World Projects](./04-real-world-projects/00-index.md)
2. [GitHub Platform](./05-github-platform/00-index.md)
3. [Next Steps](./07-next-steps/00-index.md)

### Advanced Track
1. [Git Fundamentals](./02-git-fundamentals/00-index.md)
2. [Advanced Topics](./06-advanced-topics/00-index.md)
3. [GitHub Platform](./05-github-platform/00-index.md)

> 💡 **Tip:** Set this page as your Wiki.js home for the best navigation experience.
