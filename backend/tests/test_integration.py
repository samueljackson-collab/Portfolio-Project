...
class TestUserContentWorkflow:
    ...
    async def test_full_user_workflow(self, client: AsyncClient):
        ...
        register_response = await client.post(
            "/auth/register",
            json={"email": "workflow_test@example.com", "password": "workflowpass123"},
        )
        ...
        get_response = await client.get(f"/content/{content_id}", headers=auth_headers)
        ...
        delete_response = await client.delete(
            f"/content/{content_id}", headers=auth_headers
        )
...
class TestDatabaseTransactions:
    ...
    async def test_user_can_only_modify_own_content(
        self, client: AsyncClient, test_content, test_user
    ):
        """Test that users cannot modify content they don't own."""
        # Register a different user
        register_response = await client.post(
            "/auth/register",
            json={"email": "other_user@example.com", "password": "otherpass123"},
        )
        assert register_response.status_code == 201
        ...
        login_response = await client.post(
            "/auth/login",
            data={"username": "other_user@example.com", "password": "otherpass123"},
        )
