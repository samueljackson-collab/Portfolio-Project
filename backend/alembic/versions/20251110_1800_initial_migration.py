...
def upgrade() -> None:
    ...
    op.create_table(
        'content',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False, comment='Content unique identifier'),
        sa.Column('title', sa.String(length=255), nullable=False, comment='Content title'),
        sa.Column('body', sa.Text(), nullable=True, comment='Content body text'),
        sa.Column('owner_id', postgresql.UUID(as_uuid=True), nullable=False, comment='Content owner user ID'),
        sa.Column('is_published', sa.Boolean(), nullable=False, server_default=sa.text('false'), comment='Publication status'),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False, comment='Content creation timestamp'),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False, comment='Last update timestamp'),
    ...
    op.create_index('ix_content_owner_created', 'content', ['owner_id', 'created_at'], unique=False)
